...
typedef NPT_Array<NPT_String>              PLT_StringArray;
typedef NPT_Array<PLT_DeviceDataReference>  PLT_DeviceArray;



struct eventDataRenderControlResponse
{
...
typedef NPT_List<PLT_StateVariable*>* eventDataStateVariables;
typedef void(eventNotify)( enum event_name eventName,void *result,void* event_custom_data);
...
//typedef PLT_SyncMediaBrowser PLT_SyncMediaBrowserMy;

/**
 PLT_SyncMediaBrowserMy is a copy of public PLT_SyncMediaBrowser(last update 2014-4-16).
 Only changed a default param value from "childCount" to "@childCount".
 So we should update this class if the PLT version is udpate.
 */
class PLT_SyncMediaBrowserMy : public PLT_SyncMediaBrowser
{
public:
    PLT_SyncMediaBrowserMy(PLT_CtrlPointReference&            ctrlPoint,
...
+---------------------------------------------------------------------*/
class PLT_MicroMediaController : public PLT_SyncMediaBrowserMy,
...
public:
    ...
    void HandleCmd_getVolumn();
    void HandleCmd_setVolumn(int volume);
    void HandleCmd_getPositionInfo();
    void HandleCmd_getMute();
    
public:
    void startSearchDevices();
    
    /// Get server device list.
    const PLT_DeviceArray GetServerDevices();
    
    void ChooseDeviceWithUUID(const NPT_String &uuid);
    
    const PLT_DeviceArray GetRenderDevices();
    
    PLT_MediaObjectListReference ls();
    
    PLT_MediaObjectListReference getCurBrowseResults();
    
    void cd(NPT_String objID);
    
    void selectMR(int index);
    
    /// return resource index, -1 if not finded.
    int openIndex(int index);
    
    void setEventNotifyHandler(eventNotify *eventHandler,void* custom);

private:
    void *event_custom_data;
    eventNotify *eventHandler;
    
    // playing media title
    NPT_String mediaTitle;
    
//    PLT_MediaObject * currMedia;
public:
    
    // playing index.
    int m_CurBrowseIndex;
    
    // index ready for playing
...
extern const char szObjectClassTypeImage[];
extern const char szObjectClassTypeVideo[];
extern const char szObjectClassTypeAudio[];
