...
PLT_MicroMediaController::PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint) :
    ...
    PLT_MediaController(ctrlPoint),eventHandler(NULL),m_CurBrowseIndex(-1)
{
...
{
    printf("OnMSAdded device added: %s\n",(char*) device->GetFriendlyName());
...
PLT_MicroMediaController::OnMRAdded(PLT_DeviceDataReference& device)
{
    ...
    printf("OnMRAdded device added: %s\n",(char*) device->GetFriendlyName());
...
int PLT_MicroMediaController::openIndex(int index)
{
    PLT_DeviceDataReference device;
    ...
    else
    {
        // get the protocol info to try to see in advance if a track would play on the device
        if ( m_MostRecentBrowseResults.IsNull() ||
            index >= m_MostRecentBrowseResults->GetItemCount()
            )
        {
            return -1;
        }
...
const char szObjectClassTypeImage[] = "object.item.imageItem";
const char szObjectClassTypeVideo[]= "object.item.videoItem";
const char szObjectClassTypeAudio[]= "object.item.audioItem";
