...
-(int)groupCount:(int)type
{
    ...
    else
    {
        ...
        if (iPhotoGroupCount == -1)
        {
            ...
            iPhotoGroupCount = (int)assetGroups.count;
        }
        
        
        return iPhotoGroupCount;
    }
...
//    NSLog(@"on file request: %@",theURL);
    ...
    [al assetForURL:theResourceURL resultBlock:^(ALAsset *asset) {
        ...
        if (asset && repr) {
            ...
            NSRange r =  [urlSubstring rangeOfString:@"ext="];
            if ( r.length >0 ) {
                const char *p = urlSubstring.UTF8String;
                char temp[256]="a.";
                strcpy(temp+2, p+r.location+4);
                PLT_HttpServer::ServeStream(*request, *context, *response, r_stream, PLT_MimeType::GetMimeType( temp , &tmp_context) );
            }
            else{
