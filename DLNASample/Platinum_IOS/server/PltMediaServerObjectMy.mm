                                      ...
                                      const PLT_HttpRequestContext& context)
    {
        ...
        if (  object_id[1] == 0 )
        {
            ...
            /// Browser Folders
            {
                ...
                NPT_String dir;
                NPT_FileInfo info;
                if (NPT_FAILED(fileDelegate->GetFilePath( "0" , dir)) || NPT_FAILED(NPT_File::GetInfo(dir, &info)) || info.m_Type != NPT_FileInfo::FILE_TYPE_DIRECTORY) {
                }
                
                
                NPT_LargeSize count = 0;
                if ( NPT_SUCCEEDED(NPT_File::GetSize(dir, count))) {
            ...
            /// Video
            NPT_CHECK_SEVERE(PLT_Didl::ToDidl(*item.AsPointer(), filter , tmp));
            ((PLT_MediaContainer*)object)->m_ChildrenCount = (NPT_Int32)[photoDMSDelegate groupCount:0];
            ...
            ((PLT_MediaContainer*)object)->m_ChildrenCount = (NPT_Int32)[photoDMSDelegate groupCount:1];
            object->m_ObjectID = "0/";
...
- (id)init
{
    if (self = [super init])
    {
        ...
        sDevice = PLT_DeviceHostReference (server);
        [self setDevice: &sDevice ];
...
- (id)initServerSelfDelegateWithServerName:(NSString *)theServerName
{
    if (self = [super init])
    {
        ...
        sDevice = PLT_DeviceHostReference( server );
        sDevice->m_ModelDescription = "Genie Media Server";
...
-(PLT_DeviceHostReference)getTheDevice
{
    return sDevice;
}
